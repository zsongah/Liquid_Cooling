...
(() => {
  ...
  const kindLabels = {facility_water:'设施水侧',room_air:'空气热汇',cdu:'CDU',rack:'机柜',node:'计算节点',manifold:'分配总管',pump:'泵',valve:'阀',sensor:'测点',chiller:'冷机',dry_cooler:'干冷器',cooling_tower:'冷却塔',heat_exchanger:'换热器'};
  ...
  function selectedScene(){return $('topology-source').value==='config'?(state.topologyConfig?.scene||state.scene):(state.run?.scene||state.run?.session?.scene);}
  ...
  async function refreshSite(){
    ...
    if(!id){state.site=null;state.siteConfigId='';return;}
  ...
  function renderTopology() {
    ...
    const topo=currentTopology();state.topology=topo;
    ...
    $('topology-explanation').textContent=physical?`来自配置的端口连接 · 证据：${({illustrative:'示意配置',declared:'现场声明',verified:'附核验引用'})[topo.evidence]||topo.evidence||'未填写'}`:'由控制域生成；连线不表示已测绘的管路';
    $('topology-boundary').textContent=physical?'端口连接来自配置声明，不代表水力求解或现场核验已通过。无机柜／节点测点时不生成其温度。':'当前配置只声明供冷服务与热汇关系，尚未提供完整实际管路。没有机柜／节点测点时不生成其温度。';
    const context=`${$('topology-source').value}:${$('topology-source').value==='run'?state.runId:state.topologyConfig?.id||state.config?.id}`;
    if(state.layoutContext!==context){state.layout=clone(topo.layout||{});state.layoutContext=context;state.selectedAsset=null;state.zoom='fit';}
  ...
  function nodeLevel(n){return ({facility_water:0,room_air:0,chiller:0,cooling_tower:0,dry_cooler:0,cdu:1,pump:1,heat_exchanger:1,manifold:2,valve:2,rack:3,node:4,sensor:4})[n.kind]??2;}
  // 画布坐标仅用于可视化；拖拽不会更改端口连接或控制域分配。
  // 缺少显式坐标时按资产层级布局，复杂现场可保存独立的界面布局草稿。
  function drawTopology(){
    if(!state.topology?.nodes?.length)return;
    const {nodes,edges}=state.topology,levels=[...new Set(nodes.map(nodeLevel))].sort((a,b)=>a-b),positions={};
    const nodeW=178,nodeH=94;
    for(const level of levels){const list=nodes.filter(n=>nodeLevel(n)===level);list.forEach((n,i)=>{positions[n.id]=state.layout[n.id]||{x:35+levels.indexOf(level)*230,y:65+i*130};});}
    ...
    const guides=levels.map(level=>{const list=nodes.filter(n=>nodeLevel(n)===level),x=Math.min(...list.map(n=>positions[n.id].x));return `<text class="layer-label" x="${x}" y="${top+19}">${['热汇 / 冷源','CDU / 换热','分配回路','机柜','计算节点'][level]||'设备'}</text>`;}).join('');
  ...
  function renderConfig(){const record=state.config,scene=state.scene;if(!record||!scene){$('config-form').innerHTML='<div class="empty-state">暂无配置</div>';return;}$('config-status').textContent=human(record.status);$('config-status').className=`pill ${record.status==='published'?'teal':'neutral'}`;$('config-revision').textContent=record.id?`版本 r${record.revision} · ${record.id}`:'尚未保存';$('config-readonly').hidden=editable();$('config-readonly').textContent=record.status==='template'?'当前是配置模板，尚未发布。点击“复制为新草稿”后编辑、校验并发布，才能创建运行任务。':record.status==='published'?'当前是已发布快照。点击“复制为新草稿”后编辑，正在运行的配置不受影响。':'当前配置为只读内容。复制为新草稿后再修改。';$('scene-json').value=JSON.stringify(scene,null,2);$('json-status').textContent='';$('scene-json').disabled=!editable();$('apply-json').disabled=!editable();renderBasicForm();renderAssetForm();renderValidation();updateConfigActions();}
  function renderBasicForm(){const s=state.scene,d=currentDomain(),p=currentProfile(),policy=p?.policy;const controls=Object.keys(p?.controls||{}).filter(q=>['cdu.dp_sp','cdu.sec_flow_sp'].includes(q));const disabled=editable()?'':'disabled';let html=`<div class="form-section"><h3>场景与控制域</h3><div class="form-grid">${fieldHtml('edit-name','配置名称',state.config.name,'text')}${fieldHtml('edit-scene-id','场景 ID',s.scene_id,'text')}${fieldHtml('edit-topology-version','拓扑版本',s.topology_version,'text')}<div class="field"><label for="edit-domain">控制域</label><select id="edit-domain">${(s.control_domains||[]).map(a=>`<option value="${esc(a.id)}" ${a.id===d?.id?'selected':''}>${esc(a.id)}</option>`).join('')}</select></div></div><p class="fine-print">${d?`CDU: ${esc(d.cdu_id)} · ${esc(p?.adapter||'未绑定驱动')} · 服务 ${d.served_racks?.length||0} 个机柜`:'未配置控制域'}</p></div>`;
  ...
  async function publishConfig(){if(state.dirty||state.jsonPending)throw new Error('请先保存草稿并校验。');if(!state.config.id)throw new Error('请先保存草稿。');const v=await validateConfig();if(!v.valid)throw new Error('存在配置错误，无法发布。');const yes=await confirmAction('发布当前配置版本？',`将发布“${state.config.name}” r${state.config.revision} 的已保存快照。发布不会创建任务，也不会写入设备；运行页可选择此版本启动。`);if(!yes)return;const published=await post(`/api/configs/${encodeURIComponent(state.config.id)}/publish`,{expected_revision:state.config.revision});await refreshCatalog();await loadConfig(published.id);await loadRunConfig(published.id);toast('配置已发布，可前往运行页选择该版本。');}
  function renderAssetForm(){const scene=state.scene,assets=scene.assets||[],p=scene.physical_topology,disabled=editable()?'':'disabled';const options=(values,current)=>values.map(([v,label])=>`<option value="${esc(v)}" ${v===current?'selected':''}>${esc(label)}</option>`).join('');const assetOptions=[['','无归属'],...assets.map(a=>[a.id,a.id])];
  ...
  function refreshForecastSelectors(){
    ...
    $('forecast-import').disabled=!state.forecastConfigId;$('forecast-validate').disabled=!state.forecastConfigId;
  }
  async function refreshForecast(descriptorNeeded=false){
    ...
    if(changed){state.forecastState=null;state.forecastDescriptor=null;refreshForecastSelectors();}
  ...
  function forecastPayload(){if(!state.forecastConfigId)throw new Error('请先选择已发布配置。');const text=$('forecast-json').value.trim();if(!text)throw new Error('请粘贴或选择上游预测 JSON 文件。');const payload=JSON.parse(text);if(!payload||typeof payload!=='object'||Array.isArray(payload))throw new Error('预测包必须是 JSON 对象。');if($('forecast-job').value)payload.job_id=$('forecast-job').value;else delete payload.job_id;return payload;}
  ...
  function bindEvents(){document.querySelectorAll('[data-page]').forEach(b=>b.addEventListener('click',()=>choosePage(b.dataset.page)));window.addEventListener('hashchange',()=>choosePage(location.hash.slice(1)));$('go-config').addEventListener('click',()=>choosePage('config'));
    ...
    $('topology-source').addEventListener('change',()=>action(null,async()=>{state.topologySource=$('topology-source').value;updateTopologySelection();state.layoutContext=null;if(state.topologySource==='config')await loadTopologyConfig($('topology-selection').value);else await loadRun($('topology-selection').value);}));
